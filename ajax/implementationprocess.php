...
switch($switchtype)
{
	case 'generatecustomerlist':
	{
		...
		if($coordinator == 'yes')
		{
			...
			where imp_implementation.customerreference is not null and imp_implementation.assignimplemenation is not null and (imp_implementationdays.assignimplemenation = '".$userid."' or (imp_implementation.assignimplemenation = '".$userid."' and left(imp_implementation.createddatetime,10) < '2023-03-02'))".$customerlistpiece." order by businessname;";
		}
		else
		{
			$query = "select distinct inv_mas_customer.slno,inv_mas_customer.businessname from inv_mas_customer 
			left join imp_implementation on inv_mas_customer.slno = imp_implementation.customerreference
			left join imp_implementationdays on imp_implementationdays.impref = imp_implementation.slno 
			where imp_implementation.customerreference is not null and (imp_implementationdays.assignimplemenation = '".$userid."' or (imp_implementation.assignimplemenation = '".$userid."' and left(imp_implementation.createddatetime,10) < '2023-03-02'))".$customerlistpiece." order by businessname;";
		}
	...
	case 'gridtoform':
	{
		...
		$responsearray["assignimplemenation"] = $fetch['assignimplemenation'];
	...
	case 'savestartvisit':
	{
		...
		$query = "Update imp_implementationdays set visitedstartflag = 'yes',visitedendflag = 'no',visiteddate = '".changedateformat($date)."', starttime = '".$starttime."',serverstarttime = '".date('H:i:s')."'  where slno = '".$impreflastslno."';";
		$result = runmysqlquery($query);
		$query11 = "Update imp_implementation set implementationstatus = 'progess' where imp_implementation.slno = '".$implastslno."' ";
		$resultfetch = runmysqlquery($query11);
		$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','163','".date('Y-m-d').' '.date('H:i:s')."')";
		$eventresult = runmysqlquery($eventquery);
		//Visit Started mail
		$result = sendstartvisit($impreflastslno);
	...
	case 'saveendvisit':
	{
		...
		// $query23 = "SELECT slno,visitnumber FROM imp_implementationdays WHERE impref = '".$implastslno."' AND visitnumber=(SELECT max(visitnumber) FROM imp_implementationdays WHERE impref = '".$implastslno."');";
		$query23 = "SELECT slno FROM imp_implementationdays WHERE impref = '".$implastslno."' order by slno desc limit 1;";
		$result56 = runmysqlqueryfetch($query23);
		if($result56['slno'] == $impreflastslno)
		{
			if($iccollected == 'no')
			{
		...
		if($result56['slno']!= $impreflastslno)
		{
			if($iccollected == 'yes')
			{
				echo('7^'.'ICC attachment is Compulsary only for the last visit.');
				exit;
			}
		}

		if($result56['slno'] == $impreflastslno && $impstatus!='Completed')
		{
		...
		if($iccfilepath != '')
		{
			if($iccollected == 'yes')
			{
				$query13 = "Update imp_implementationdays set iccattachment = 'yes' where impref = '".$implastslno."';";
				$result11 = runmysqlquery($query13);
				$query15 = "Update imp_implementation set implementationstatus = 'completed' where imp_implementation.slno = '".$implastslno."' ";
				$resultfetch = runmysqlquery($query15); 
			
			}
			
			$query11 = "Update imp_implementationdays set visitedstartflag = 'yes',visitedendflag = 'yes',iccattachmentpath = '".$iccfilepath."',iccattachmentdate ='".date('Y-m-d').' '.date('H:i:s')."', iccattachmentby = '".$userid."',databackuppath = '".$databackuppath."',databackupdate ='".date('Y-m-d').' '.date('H:i:s')."', databackupby = '".$userid."',impstatus='".$impstatus."' where slno = '".$impreflastslno."';"; //echo($query11);exit;
			$result11 = runmysqlquery($query11); 
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','164','".date('Y-m-d').' '.date('H:i:s')."')";
			$eventresult = runmysqlquery($eventquery);
			for($j=0;$j<$countactivity;$j++)
			{
				$queryres = "Update imp_activitycarried set activityref = '".$impreflastslno."' where slno = '".$slnoactivity[$j]."' ";
				$result11 = runmysqlquery($queryres);
				
			}
			if($activitydeleteslno != '')
			{
				$queryres ="Delete from imp_activitycarried where slno = '".$activitydeleteslno."';";
				$result11 = runmysqlquery($queryres);
			}
			if($iccollected == 'yes')
			{
				$result = sendicccollectedmail($impreflastslno); 
			}
		}
		else
		{
			$query = "Update imp_implementationdays set dayremarks = '".$dayremarks."', endtime = '".$endtime."', visitedstartflag = 'yes',visitedendflag = 'yes',serverendtime = '".date('H:i:s')."',databackuppath = '".$databackuppath."',databackupdate ='".date('Y-m-d').' '.date('H:i:s')."', databackupby = '".$userid."',impstatus='".$impstatus."'  where slno = '".$impreflastslno."';";
			$result = runmysqlquery($query);
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','164','".date('Y-m-d').' '.date('H:i:s')."')";
			$eventresult = runmysqlquery($eventquery);
			...
			$result = sendendvisit($impreflastslno);
		}
	...
	/*case 'generatevisitgrid':
	{
		...
		$query = "SELECT slno,assigneddate,remarks,dayremarks from imp_implementationdays  WHERE impref = '".$implastslno."' order by createddatetime DESC LIMIT ".$startlimit.",".$limit.";";
		if($startlimit == 0)
		{
			$grid = '<table width="100%" cellpadding="3" cellspacing="0" class="table-border-grid">';
			$grid .= '<tr class="tr-grid-header" align ="left"><td nowrap = "nowrap" class="td-border-grid" >Sl No</td><td nowrap = "nowrap" class="td-border-grid">Date</td><td nowrap = "nowrap" class="td-border-grid">Visit Remarks</td><td nowrap = "nowrap" class="td-border-grid">Activity Remarks</td></tr>';
		}
		$i_n = 0;
		$result = runmysqlquery($query);
		while($fetch = mysqli_fetch_array($result))
		{
			...
			if($i_n%2 == 0)
				...
				$color = "#f7faff";
			$grid .= '<tr class="gridrow" bgcolor='.$color.' onclick="visitsgridtoform(\''.$fetch['slno'].'\'); " align ="left">';
			...
			$grid .= "<td nowrap='nowrap' class='td-border-grid'>".$fetch['dayremarks']."</td>";
	...
	case 'generatevisitgrid':
	{
		...
		left join imp_implementationdays on imp_implementation.slno = imp_implementationdays.impref where imp_implementation.slno = '".$implastslno."'";
		$fetch1 = runmysqlqueryfetch($query1);

		$query11 = "SELECT slno,assigneddate,remarks,dayremarks,visitnumber,assignimplemenation,left(createddatetime,10) as assignedcreated from imp_implementationdays  WHERE impref = '".$implastslno."' order by createddatetime asc; ";
		$result11 = runmysqlquery($query11);
	...
	case 'visitsgridtoform':
	{
		...
		$query = "select inv_mas_implementer.businessname,assigneddate,visitedstartflag,visitedendflag ,iccattachment,TIME_FORMAT(starttime,'%h:%i:%p') as starttime,TIME_FORMAT(endtime,'%h:%i:%p') as endtime,dayremarks,impref,visiteddate,iccattachmentpath,databackuppath,impstatus from imp_implementationdays left join imp_implementation on imp_implementationdays.impref = imp_implementation.slno  left join inv_mas_implementer on inv_mas_implementer.slno = imp_implementation.assignimplemenation   where imp_implementationdays.slno = '".$impreflastslno."';";
		//echo($query);exit;
		$fetch = runmysqlqueryfetch($query);

		$query1 = "select count(*) as visitflagcount from imp_implementationdays where impref = '".$fetch['impref']."' and assignimplemenation='".$userid."' and slno < '".$impreflastslno."'  and visitedstartflag='no'";
		$fetch1 = runmysqlqueryfetch($query1);
		$count = $fetch1['visitflagcount'];
		if($count == 0)
		{
			/*$query1 = "select *, max(slno) from imp_implementationdays where impref = '".$fetch['impref']."' group by impref;";
	...
	case 'saveactivity':
	{
		...
		{
			$query = "insert imp_activitycarried ( activity, description, createddatetime,createdby, createdip, 
			...
			$result = runmysqlquery($query);
			//exit;
			$query12 = "SELECT slno AS slno FROM imp_activitycarried where impref = '".$implastslno."' and (imp_activitycarried.activityref is null or imp_activitycarried.activityref = '')";
		...
		{
			$query = "Update imp_activitycarried set description = '".$description."', activity = '".$activity."' , lastupdateddatetime = '".date('Y-m-d').' '.date('H:i:s')."',lastupdatedip = '".$_SERVER['REMOTE_ADDR']."',lastupdatedby = '".$userid."' where slno = '".$impreflastslno."';";
			$result = runmysqlquery($query);
			$query12 = "SELECT slno AS slno FROM imp_activitycarried where impref = '".$implastslno."'  and (imp_activitycarried.activityref is null or imp_activitycarried.activityref = '')";
			$result222 = runmysqlquery($query12);
			while($fetch = mysqli_fetch_array($result222))
	...
	{
		$impreflastslno = $_POST['impreflastslno'];
		$resultcount = "SELECT count(*) as count  from imp_activitycarried left join imp_mas_activity on imp_mas_activity.slno = imp_activitycarried.activity where imp_activitycarried.impref = '".$impreflastslno."' and (imp_activitycarried.activityref is null or imp_activitycarried.activityref = '');";
		$fetch10 = runmysqlqueryfetch($resultcount);
		$fetchresultcount = $fetch10['count'];
		...
		{
			$query = "SELECT imp_activitycarried.slno,imp_mas_activity.activityname,imp_activitycarried.description from imp_activitycarried left join imp_mas_activity on imp_mas_activity.slno = imp_activitycarried.activity where imp_activitycarried.impref = '".$impreflastslno."' and (imp_activitycarried.activityref is null or imp_activitycarried.activityref = '') order by createddatetime DESC";
			$result = runmysqlquery($query);
			$i_n = 0;
			while($fetch = mysqli_fetch_array($result))
			{
	...
	{
		$impactivitylastslno = $_POST['impactivitylastslno'];
		$query = "SELECT imp_activitycarried.activity, imp_activitycarried.description, imp_implementationactivity.remarks from imp_activitycarried 
		left join imp_implementationactivity on imp_implementationactivity.activity = imp_activitycarried.activity  where  imp_activitycarried.slno = '".$impactivitylastslno."';";
		$fetch = runmysqlqueryfetch($query);
	...
	case 'activitygridvisit':
	{
		...
		$query = "SELECT count(*) as count  from imp_activitycarried where (imp_activitycarried.activityref is null or imp_activitycarried.activityref = '')";
		$resfetch = runmysqlqueryfetch($query);
		$resultcountfetch = $resfetch['count'];
		if($resultcountfetch != 0)
		{
			$queryres ="Delete from imp_activitycarried where (imp_activitycarried.activityref is null or imp_activitycarried.activityref = '');";
			$result11 = runmysqlquery($queryres);
		}
		
		$resultcount = "SELECT count(*) as count  from imp_activitycarried left join imp_mas_activity on imp_mas_activity.slno = imp_activitycarried.activity where imp_activitycarried.activityref = '".$activityref."';";
		$fetch10 = runmysqlqueryfetch($resultcount);
		$fetchresultcount = $fetch10['count'];
		...
		if($fetchresultcount > 0)
		{
			$query = "SELECT imp_activitycarried.slno,imp_mas_activity.activityname,imp_activitycarried.description from imp_activitycarried left join imp_mas_activity on imp_mas_activity.slno = imp_activitycarried.activity where imp_activitycarried.activityref = '".$activityref."' order by createddatetime DESC";
			$result = runmysqlquery($query);
			$i_n = 0;
			while($fetch = mysqli_fetch_array($result))
			{
	...
	case 'deleteactivity':
	{
		...
		$impactivitylastslno = $_POST['impactivitylastslno'];
		//$query = "Delete from imp_activitycarried where slno = '".$impactivitylastslno."';";
		//$result = runmysqlquery($query);
	...
	case 'getactivityremarks':
	{
		...
		$query = "select * from imp_implementationactivity where activity = '".$activity."' and impref = '".$implastslno."';";
	...
	case 'implemenationstatus':
	{
		...
		imp_implementation.implementationstatus, inv_mas_implementer.businessname, imp_implementation.advancecollected ,
		imp_implementation.advancesnotcollectedremarks ,imp_implementationdays.assignimplemenation as assignedimp
		from  imp_implementation 
		left join inv_mas_implementer on inv_mas_implementer.slno = imp_implementation.assignimplemenation 
		left join imp_implementationdays on inv_mas_implementer.slno = imp_implementationdays.assignimplemenation
		where imp_implementation.slno = '".$lastslno."';";
		$fetch = runmysqlqueryfetch($query);
		
		$query1 = "Select assigneddate from imp_implementationdays where imp_implementationdays.impref = '".$lastslno."' and assignimplemenation = '".$fetch['assignedimp']."';";
	...
	case 'visitdetails':
	{
		...
		$query1 = "select distinct left(imp_implementation.createddatetime,10) as impcreated from imp_implementation
		left join imp_implementationdays on imp_implementation.slno = imp_implementationdays.impref where imp_implementation.slno = '".$impslno."'";
		$fetch1 = runmysqlqueryfetch($query1);

		$query11 = "SELECT slno,assigneddate,remarks,dayremarks,assignimplemenation from imp_implementationdays  WHERE impref = '".$impslno."' order by createddatetime asc ";
		$result11 = runmysqlquery($query11);
		$grid .= '<select name="activitygrid" class="swiftselect" id="activitygrid" style="width:200px;" ><option value="" selected="selected">-- Select--</option>';
		while($fetch = mysqli_fetch_array($result11))
		{
	...
	case 'filtercustomerlist':
	{
		...
		if($status == "")
		{
			...
			{
				$query = $querycase." where imp_implementation.customerreference is not null and (imp_implementationdays.assignimplemenation = '".$userid."' or (imp_implementation.assignimplemenation = '".$userid."' and left(imp_implementation.createddatetime,10) < '2023-03-02'))".$customerlistpiece." order by businessname;";
			}
			$result = runmysqlquery($query);
			$count = 0;
			while($fetch = mysqli_fetch_array($result))
		...
		else
		{
			...
			while($fetch0 = mysqli_fetch_array($result0))
			{
				if($coordinator == 'yes')
				{
					...
					left join imp_implementation on inv_mas_customer.slno = imp_implementation.customerreference 
					left join imp_implementationdays on imp_implementationdays.impref = imp_implementation.slno
					where inv_mas_customer.slno = '".$fetch0['customerreference']."' and imp_implementation.slno = '".$fetch0['impslno']."' ".$statuspiece." and  imp_implementation.assignimplemenation is not null and (imp_implementationdays.assignimplemenation = '".$userid."' or (imp_implementation.assignimplemenation = '".$userid."' and left(imp_implementation.createddatetime,10) < '2023-03-02')) order by businessname;";
				}
				else
				{
					$query = "select distinct inv_mas_customer.slno,inv_mas_customer.businessname from inv_mas_customer 
					left join imp_implementation on inv_mas_customer.slno = imp_implementation.customerreference
					left join imp_implementationdays on imp_implementationdays.impref = imp_implementation.slno 
					where inv_mas_customer.slno = '".$fetch0['customerreference']."' and imp_implementation.slno = '".$fetch0['impslno']."' ".$statuspiece." and (imp_implementationdays.assignimplemenation = '".$userid."' or (imp_implementation.assignimplemenation = '".$userid."' and left(imp_implementation.createddatetime,10) < '2023-03-02')) order by businessname;";
				}
	...
	case 'searchcustomerlist':
	{
		...
		$coordinator = $resultfetch['coordinator'];
		if($coordinator == 'yes')
			$coordinatorpiece ="and imp_implementation.assignimplemenation is not null";
		else
			$coordinatorpiece = "and imp_implementation.assignimplemenation = '".$userid."'";
