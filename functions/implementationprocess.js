...
function customercontactdetailstoform(cusid)
{
	if(cusid != '')
	{
		...
		$('#lastslno').val(cusid);
		var queryString = "../ajax/implementationprocess.php";
		...
		ajaxcall5 = $.ajax(
		{
			...
			{	
					...
					if(response['errorcode'] == '1')
					{
						...
						if(response['assignimplemenation'] != null || response['assignimplemenation'] != '')
						{
...
function visitupdate(command) 
{
	...
	var field = $('#visit_endtime');
		...
		//if(field.val()) { if(!validatetime(field.val())) { error.html(errormessage('Enter a Valid Time.')); field.focus(); return false; } }
